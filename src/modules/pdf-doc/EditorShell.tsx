...
interface EditorShellProps {
  ...
  /**
   * If this PDF is already an owned, saved document (opened from the
   * dashboard rather than a fresh guest upload), pass its id here. This
   * skips the sign-in + "save as new document" steps in the share dialog
   * (it's already saved and owned) and enables the "Save changes" button
   * so edits persist back to the same document instead of only being
   * available as a local download.
   */
  documentId?: string;
  /** Route to navigate back to. Defaults to "/". */
  backTo?: string;
  /** Called after a successful save-to-storage, e.g. to invalidate a query. */
  onSaved?: () => void;
  /** Async persist function for "Save changes". Required when documentId is set. */
  saveDocument?: (bytes: Uint8Array, pageCount: number) => Promise<void>;
}
...
export function EditorShell({
  ...
  async function handleSave() {
    ...
    try {
      const bytes = await exportPdf(state);
      await saveDocument(bytes, state.pages.length);
  ...
  return (
    <div className="flex h-screen flex-col bg-background">
      ...
      <ShareTrackDialog
        ...
        getPdfBytes={documentId ? undefined : getExportedBytes}
        pageCount={documentId ? undefined : state.pages.length}
        docName={docName}
        documentId={documentId}
      />

      <div className="flex flex-1 overflow-hidden">
        <ThumbnailSidebar selectedId={selectedPageId} onSelect={setSelectedPageId} />
